...
doc_type: sop
title: 熔断降级 SOP
filename: sop_circuit_breaker.md
severity: info
related_services:
  - order-service
  - payment-service
...
本 SOP 适用于智能 OnCall 系统在处理 下游服务不可用、调用超时放大、重试导致级联故障 时的标准化操作。SOP 的目标不是替代人工判断，而是帮助值班人员在压力场景下快速完成止血、确认、通知和复盘。AIOps 可以在诊断报告中引用本 SOP，给出符合流程的应急建议。
...
第一步，确认下游异常是否持续。第二步，开启熔断或返回兜底结果。第三步，关闭高成本重试。第四步，观察上游错误率是否下降。第五步，待下游恢复后逐步关闭降级。

## 4. 风险控制

降级可能导致功能不可用或数据延迟，应明确降级范围，并避免把异常结果写入核心数据。所有应急操作都需要记录执行人、执行时间、目标服务、变更内容和观察结果。对于可能影响用户体验的降级、限流或回滚操作，需要在通知中说明影响范围和预计恢复时间。
