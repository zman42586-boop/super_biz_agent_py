...
doc_type: sop
title: 故障复盘 SOP
filename: sop_postmortem_template.md
severity: info
related_services:
  - order-service
  - payment-service
...
本 SOP 适用于智能 OnCall 系统在处理 故障恢复后需要沉淀经验、完善监控和更新知识库 时的标准化操作。SOP 的目标不是替代人工判断，而是帮助值班人员在压力场景下快速完成止血、确认、通知和复盘。AIOps 可以在诊断报告中引用本 SOP，给出符合流程的应急建议。
...
第一步，整理事件时间线。第二步，列出影响范围和用户损失。第三步，明确根因和证据。第四步，记录有效和无效操作。第五步，制定预防措施并更新 runbook。

## 4. 风险控制

复盘应避免归因到个人操作，而应聚焦流程、系统、监控和自动化能力改进。所有应急操作都需要记录执行人、执行时间、目标服务、变更内容和观察结果。对于可能影响用户体验的降级、限流或回滚操作，需要在通知中说明影响范围和预计恢复时间。
...
补充说明：若同类事件再次出现，应优先比对本次指标和历史事件差异。