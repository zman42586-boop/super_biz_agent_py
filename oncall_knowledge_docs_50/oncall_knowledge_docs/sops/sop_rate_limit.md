...
本 SOP 适用于智能 OnCall 系统在处理 错误率升高、下游超时、热点接口被突发流量打满 时的标准化操作。SOP 的目标不是替代人工判断，而是帮助值班人员在压力场景下快速完成止血、确认、通知和复盘。AIOps 可以在诊断报告中引用本 SOP，给出符合流程的应急建议。
...
第一步，确定需要限流的接口、用户或来源。第二步，选择限流阈值并灰度生效。第三步，观察错误率和业务影响。第四步，保留核心链路，限制非核心请求。第五步，故障恢复后逐步放开。

## 4. 风险控制

限流会影响部分用户请求，应优先保护支付、下单、库存扣减等核心链路，并在通知中说明影响范围。所有应急操作都需要记录执行人、执行时间、目标服务、变更内容和观察结果。对于可能影响用户体验的降级、限流或回滚操作，需要在通知中说明影响范围和预计恢复时间。
