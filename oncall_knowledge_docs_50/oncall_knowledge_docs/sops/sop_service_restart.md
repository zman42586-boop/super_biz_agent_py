...
doc_type: sop
title: 服务重启 SOP
filename: sop_service_restart.md
severity: info
related_services:
  - order-service
  - payment-service
...
本 SOP 适用于智能 OnCall 系统在处理 单实例异常、内存持续上涨、线程卡死或健康检查失败 时的标准化操作。SOP 的目标不是替代人工判断，而是帮助值班人员在压力场景下快速完成止血、确认、通知和复盘。AIOps 可以在诊断报告中引用本 SOP，给出符合流程的应急建议。
...
第一步，确认是否为单实例问题。第二步，摘除异常实例流量。第三步，保存关键日志和监控快照。第四步，执行滚动重启。第五步，恢复流量并持续观察。

## 4. 风险控制

不能在未确认影响范围时同时重启全部实例，否则可能造成短时间服务不可用或连接风暴。所有应急操作都需要记录执行人、执行时间、目标服务、变更内容和观察结果。对于可能影响用户体验的降级、限流或回滚操作，需要在通知中说明影响范围和预计恢复时间。
