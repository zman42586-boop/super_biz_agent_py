---
doc_type: incident
title: 2026-04-28 inventory-service 锁竞争事件复盘
filename: incident_2026_04_28_inventory_lock_contention.md
...
# 2026-04-28 inventory-service 锁竞争事件复盘

## 1. 事件概述

本事件为模拟历史故障复盘，涉及服务 inventory-service，主要告警为 SlowResponse。事件发生期间，用户请求出现不同程度的响应变慢或失败，AIOps 通过知识库检索、监控指标和日志分析生成初步诊断。该文档用于帮助后续相似告警进行经验复用。

## 2. 时间线

12:00 活动开始；12:03 库存接口 P95 升高；12:08 日志出现 lock wait；12:20 对热点商品限流；12:40 响应恢复
...
最终判断根因为：热点商品库存扣减使用粗粒度分布式锁，高并发下大量请求等待，导致接口响应变慢。关键证据包括日志中的异常关键词、监控曲线的时间一致性、服务依赖关系和处理动作后的恢复情况。如果证据不足，报告中应明确“高度怀疑”而不是“确定根因”。

## 5. 处理过程

对热点商品启用排队扣减和限流，减少锁持有时间，并拆分库存锁粒度。处理过程中遵循先止血后定位原则，优先降低用户影响，再逐步收集证据。对于核心服务，必要时执行限流、降级、扩容或回滚。所有动作都需要记录时间点，便于复盘。
...
活动前识别热点商品，预热库存缓存，并完善分布式锁竞争文档。此外，应将本事件沉淀到知识库，使后续 AIOps 在遇到相似告警时能够优先召回历史 incident，并结合当前监控数据生成更准确的诊断计划。
