---
doc_type: incident
title: 2026-04-18 order-service 慢查询事件复盘
filename: incident_2026_04_18_mysql_slow_query.md
...
# 2026-04-18 order-service 慢查询事件复盘

## 1. 事件概述

本事件为模拟历史故障复盘，涉及服务 order-service，主要告警为 SlowResponse。事件发生期间，用户请求出现不同程度的响应变慢或失败，AIOps 通过知识库检索、监控指标和日志分析生成初步诊断。该文档用于帮助后续相似告警进行经验复用。

## 2. 时间线

20:00 新版本发布；20:08 P95 升高；20:15 query_logs 命中 slow query；20:25 回滚搜索条件；20:45 响应时间恢复
...
最终判断根因为：新上线的订单搜索条件导致组合索引失效，数据库扫描行数增加，接口响应变慢。关键证据包括日志中的异常关键词、监控曲线的时间一致性、服务依赖关系和处理动作后的恢复情况。如果证据不足，报告中应明确“高度怀疑”而不是“确定根因”。

## 5. 处理过程

回滚相关搜索功能，临时关闭高级筛选入口，补充 SQL 审核和压测。处理过程中遵循先止血后定位原则，优先降低用户影响，再逐步收集证据。对于核心服务，必要时执行限流、降级、扩容或回滚。所有动作都需要记录时间点，便于复盘。
...
发布前增加 SQL explain 检查，将慢查询阈值接入告警。此外，应将本事件沉淀到知识库，使后续 AIOps 在遇到相似告警时能够优先召回历史 incident，并结合当前监控数据生成更准确的诊断计划。
