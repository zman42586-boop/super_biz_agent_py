---
doc_type: incident
title: 2026-04-01 order-service CPU 升高事件复盘
filename: incident_2026_04_01_order_cpu_high.md
...
# 2026-04-01 order-service CPU 升高事件复盘

## 1. 事件概述

本事件为模拟历史故障复盘，涉及服务 order-service，主要告警为 HighCPUUsage。事件发生期间，用户请求出现不同程度的响应变慢或失败，AIOps 通过知识库检索、监控指标和日志分析生成初步诊断。该文档用于帮助后续相似告警进行经验复用。

## 2. 时间线

10:05 网关错误率轻微升高；10:08 order-service CPU 超过 85%；10:12 AIOps 发现 slow query WARN 增多；10:20 临时降级深分页查询；10:35 指标恢复
...
受影响范围主要集中在 order-service 相关业务链路。若该服务位于核心路径，例如订单创建、支付确认、库存扣减或网关转发，则需要同步评估上游请求失败率和下游依赖压力。事件期间应避免重复触发大规模重试，以免进一步放大故障。

## 4. 根因分析

最终判断根因为：订单列表查询缺少合适索引，导致慢 SQL 增多，应用线程长时间等待数据库返回，CPU 和响应时间同步上升。关键证据包括日志中的异常关键词、监控曲线的时间一致性、服务依赖关系和处理动作后的恢复情况。如果证据不足，报告中应明确“高度怀疑”而不是“确定根因”。

## 5. 处理过程

临时关闭高成本筛选条件，增加缓存保护，并由数据库负责人补充索引；随后重新发布查询优化版本。处理过程中遵循先止血后定位原则，优先降低用户影响，再逐步收集证据。对于核心服务，必要时执行限流、降级、扩容或回滚。所有动作都需要记录时间点，便于复盘。
...
为订单列表接口增加慢查询监控，限制深分页，更新 runbook_high_cpu_usage.md 和 MySQL 慢查询文档。此外，应将本事件沉淀到知识库，使后续 AIOps 在遇到相似告警时能够优先召回历史 incident，并结合当前监控数据生成更准确的诊断计划。
