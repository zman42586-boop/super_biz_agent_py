---
doc_type: incident
title: 2026-04-20 inventory-service 不可用事件复盘
filename: incident_2026_04_20_inventory_unavailable.md
...
# 2026-04-20 inventory-service 不可用事件复盘

## 1. 事件概述

本事件为模拟历史故障复盘，涉及服务 inventory-service，主要告警为 ServiceUnavailable。事件发生期间，用户请求出现不同程度的响应变慢或失败，AIOps 通过知识库检索、监控指标和日志分析生成初步诊断。该文档用于帮助后续相似告警进行经验复用。

## 2. 时间线

08:40 配置变更；08:45 实例重启失败；08:48 服务不可用告警；08:55 回滚配置；09:05 实例恢复
...
最终判断根因为：配置中心下发错误 Redis 地址，导致库存服务启动后无法连接缓存并健康检查失败。关键证据包括日志中的异常关键词、监控曲线的时间一致性、服务依赖关系和处理动作后的恢复情况。如果证据不足，报告中应明确“高度怀疑”而不是“确定根因”。

## 5. 处理过程

回滚错误配置，重启受影响实例，并补充配置变更审批。处理过程中遵循先止血后定位原则，优先降低用户影响，再逐步收集证据。对于核心服务，必要时执行限流、降级、扩容或回滚。所有动作都需要记录时间点，便于复盘。
...
关键配置增加灰度校验，启动探针中区分依赖不可达和应用崩溃。此外，应将本事件沉淀到知识库，使后续 AIOps 在遇到相似告警时能够优先召回历史 incident，并结合当前监控数据生成更准确的诊断计划。
