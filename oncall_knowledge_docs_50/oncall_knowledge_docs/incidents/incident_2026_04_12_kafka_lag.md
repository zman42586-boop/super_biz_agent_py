---
doc_type: incident
title: 2026-04-12 payment-service 消息积压事件复盘
filename: incident_2026_04_12_kafka_lag.md
...
# 2026-04-12 payment-service 消息积压事件复盘

## 1. 事件概述

本事件为模拟历史故障复盘，涉及服务 payment-service，主要告警为 MessageBacklog。事件发生期间，用户请求出现不同程度的响应变慢或失败，AIOps 通过知识库检索、监控指标和日志分析生成初步诊断。该文档用于帮助后续相似告警进行经验复用。

## 2. 时间线

16:10 lag 开始上升；16:20 支付结果延迟；16:25 日志出现 order-service timeout；16:35 扩容消费者；16:50 lag 开始下降
...
最终判断根因为：支付结果通知消费者处理逻辑变慢，单条消息调用 order-service 超时，导致 Kafka lag 持续增加。关键证据包括日志中的异常关键词、监控曲线的时间一致性、服务依赖关系和处理动作后的恢复情况。如果证据不足，报告中应明确“高度怀疑”而不是“确定根因”。

## 5. 处理过程

扩容消费者实例，降低单条消息下游调用超时时间，并对失败消息进入重试队列。处理过程中遵循先止血后定位原则，优先降低用户影响，再逐步收集证据。对于核心服务，必要时执行限流、降级、扩容或回滚。所有动作都需要记录时间点，便于复盘。
...
为消费者增加处理耗时指标，拆分慢下游调用，并优化消息重试策略。此外，应将本事件沉淀到知识库，使后续 AIOps 在遇到相似告警时能够优先召回历史 incident，并结合当前监控数据生成更准确的诊断计划。
