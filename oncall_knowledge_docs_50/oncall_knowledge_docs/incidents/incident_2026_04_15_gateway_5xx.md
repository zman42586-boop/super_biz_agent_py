---
doc_type: incident
title: 2026-04-15 gateway-service 5xx 升高事件复盘
filename: incident_2026_04_15_gateway_5xx.md
...
# 2026-04-15 gateway-service 5xx 升高事件复盘

## 1. 事件概述

本事件为模拟历史故障复盘，涉及服务 gateway-service，主要告警为 HighErrorRate。事件发生期间，用户请求出现不同程度的响应变慢或失败，AIOps 通过知识库检索、监控指标和日志分析生成初步诊断。该文档用于帮助后续相似告警进行经验复用。

## 2. 时间线

11:00 网关 5xx 升高；11:04 AIOps 按路由聚合发现库存接口错误集中；11:10 摘除异常实例；11:18 5xx 恢复
...
最终判断根因为：inventory-service 部分实例健康检查异常，gateway-service 仍继续转发请求到异常实例。关键证据包括日志中的异常关键词、监控曲线的时间一致性、服务依赖关系和处理动作后的恢复情况。如果证据不足，报告中应明确“高度怀疑”而不是“确定根因”。

## 5. 处理过程

手动摘除异常 inventory-service 实例，修正健康检查配置，并刷新网关路由缓存。处理过程中遵循先止血后定位原则，优先降低用户影响，再逐步收集证据。对于核心服务，必要时执行限流、降级、扩容或回滚。所有动作都需要记录时间点，便于复盘。
...
提高健康检查准确性，完善网关 5xx runbook 和服务不可用 SOP。此外，应将本事件沉淀到知识库，使后续 AIOps 在遇到相似告警时能够优先召回历史 incident，并结合当前监控数据生成更准确的诊断计划。
