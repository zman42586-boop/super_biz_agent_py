...
filename: service_payment_callback.md
severity: info
related_services:
  - payment-service
  - order-service
...
payment-service 是智能 OnCall 演示环境中的核心业务服务之一，主要负责支付回调接收、签名校验、幂等处理和订单支付状态更新。在故障诊断中，服务画像用于帮助 AIOps 判断告警影响范围、上下游依赖和优先排查方向。Planner 在接收到某个服务相关的告警后，应优先检索本服务文档，再结合对应 runbook 和历史 incident 制定排查计划。
...
payment-service 的常见风险包括：回调流量突增；幂等锁冲突；第三方重复通知；数据库事务过长。当出现接口超时或错误率升高时，不应只看本服务日志，还应关联上下游调用关系。例如 order-service 的创建订单失败，可能来自库存扣减超时、支付预创建失败或数据库慢查询；gateway-service 的 5xx 增加，也可能只是下游服务不可用的表现。
