...
doc_type: middleware
title: 容器重启异常排查指南
filename: middleware_container_restart.md
severity: warning
related_services:
  - inventory-service
related_tools:
...
Container Runtime 是 inventory-service 业务链路中的关键依赖。当 容器频繁重启或健康检查失败 出现时，业务服务可能表现为接口响应变慢、错误率升高、线程池堆积、CPU 升高或内存压力增加。AIOps 诊断时不能只看服务自身指标，还要把中间件状态和服务日志进行关联。
...
判断重启是否由 OOM、启动探针失败、配置错误或依赖不可达导致；需要对比发布记录。如果异常只发生在某个服务，应优先检查该服务调用中间件的配置、连接池和请求模式；如果多个服务同时异常，则应优先判断中间件整体状态或网络链路问题。对于偶发超时，需要结合重试次数和请求高峰判断是否存在雪崩风险。

## 5. 应急措施

回滚异常版本，调整探针阈值，保存启动日志，必要时提高资源限制。应急过程中需要避免盲目重启所有服务，因为重启可能导致缓存失效、连接风暴或消息重复消费。建议先隔离异常实例、降低流量、限制非核心功能，再逐步恢复。
...
当 Planner 识别到 Container Runtime 相关关键词时，应同时检索服务画像、对应 runbook 和历史 incident。Executor 每次工具调用后只保留摘要，原始结果落盘，避免把大量日志直接放入上下文。
