---
doc_type: middleware
title: Redis 内存压力排查指南
filename: middleware_redis_memory_high.md
severity: warning
related_services:
  - inventory-service
related_tools:
...
Redis 是 inventory-service 业务链路中的关键依赖。当 Redis 内存使用率升高、延迟上升或淘汰增加 出现时，业务服务可能表现为接口响应变慢、错误率升高、线程池堆积、CPU 升高或内存压力增加。AIOps 诊断时不能只看服务自身指标，还要把中间件状态和服务日志进行关联。
...
需要区分是大 key、过期策略失效、写入流量突增还是缓存数据结构设计不合理。如果异常只发生在某个服务，应优先检查该服务调用中间件的配置、连接池和请求模式；如果多个服务同时异常，则应优先判断中间件整体状态或网络链路问题。对于偶发超时，需要结合重试次数和请求高峰判断是否存在雪崩风险。

## 5. 应急措施

暂停非核心缓存写入，清理过期大 key，必要时扩容 Redis 或拆分热点数据。应急过程中需要避免盲目重启所有服务，因为重启可能导致缓存失效、连接风暴或消息重复消费。建议先隔离异常实例、降低流量、限制非核心功能，再逐步恢复。
