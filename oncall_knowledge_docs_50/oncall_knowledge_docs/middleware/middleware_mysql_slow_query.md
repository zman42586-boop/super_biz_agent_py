...
doc_type: middleware
title: MySQL 慢查询排查指南
filename: middleware_mysql_slow_query.md
severity: warning
related_services:
  - order-service
related_tools:
...
MySQL 是 order-service 业务链路中的关键依赖。当 慢 SQL 或事务等待 出现时，业务服务可能表现为接口响应变慢、错误率升高、线程池堆积、CPU 升高或内存压力增加。AIOps 诊断时不能只看服务自身指标，还要把中间件状态和服务日志进行关联。
...
需要重点比较慢查询出现时间与 CPU、接口延迟的上升时间是否一致；如果慢查询集中在订单列表或库存扣减接口，应检查索引、分页条件和事务范围。如果异常只发生在某个服务，应优先检查该服务调用中间件的配置、连接池和请求模式；如果多个服务同时异常，则应优先判断中间件整体状态或网络链路问题。对于偶发超时，需要结合重试次数和请求高峰判断是否存在雪崩风险。

## 5. 应急措施

临时降级高成本查询，增加缓存保护，必要时回滚最近 SQL 变更或添加临时索引。应急过程中需要避免盲目重启所有服务，因为重启可能导致缓存失效、连接风暴或消息重复消费。建议先隔离异常实例、降低流量、限制非核心功能，再逐步恢复。
