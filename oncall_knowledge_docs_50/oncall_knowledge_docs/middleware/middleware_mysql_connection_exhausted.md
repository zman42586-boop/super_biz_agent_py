---
doc_type: middleware
title: MySQL 连接池耗尽排查指南
filename: middleware_mysql_connection_exhausted.md
severity: warning
related_services:
  - payment-service
related_tools:
...
MySQL 是 payment-service 业务链路中的关键依赖。当 连接等待、获取连接超时或事务堆积 出现时，业务服务可能表现为接口响应变慢、错误率升高、线程池堆积、CPU 升高或内存压力增加。AIOps 诊断时不能只看服务自身指标，还要把中间件状态和服务日志进行关联。
...
判断连接数耗尽是数据库端达到上限，还是应用端连接未释放；如果日志出现 connection timeout 和 slow query 同时增加，应优先处理慢查询。如果异常只发生在某个服务，应优先检查该服务调用中间件的配置、连接池和请求模式；如果多个服务同时异常，则应优先判断中间件整体状态或网络链路问题。对于偶发超时，需要结合重试次数和请求高峰判断是否存在雪崩风险。

## 5. 应急措施

限制高并发写接口，缩短异常事务，必要时滚动重启异常实例并保留连接池指标。应急过程中需要避免盲目重启所有服务，因为重启可能导致缓存失效、连接风暴或消息重复消费。建议先隔离异常实例、降低流量、限制非核心功能，再逐步恢复。
...
当 Planner 识别到 MySQL 相关关键词时，应同时检索服务画像、对应 runbook 和历史 incident。Executor 每次工具调用后只保留摘要，原始结果落盘，避免把大量日志直接放入上下文。
