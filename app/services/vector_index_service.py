...
class VectorIndexService:
    ...
    def _preprocess_content(self, content: str, source: str) -> str:
        """入库前预处理：删除演示/模拟段落与重复模板模块。

        目标：
        - 删除包含“模拟运维知识库内容/用于演示/用于验证”等提示语的段落
        - 删除“检索与演示建议”章节（通常是模板化内容，容易污染检索）
        """
        if not content:
            return content
        ...
        skip_demo_section = False

        # 章节标题匹配（只处理 markdown 二级标题的“检索与演示建议”模块）
        demo_section_title = re.compile(r"^\s*##\s*\d+\.?\s*检索与演示建议\s*$")

        # 常见的“模拟/演示”提示语（按行过滤，避免遗漏）
        boilerplate_markers = (
            "模拟运维知识库内容",
            "用于演示",
            "用于验证 RAG",
            "用于验证向量检索",
            "用于验证 RAG 检索",
            "simulated_oncall_knowledge_base",
        )

        for line in lines:
            # 进入/退出“检索与演示建议”章节
            if demo_section_title.match(line):
                skip_demo_section = True
                continue
            if skip_demo_section:
                # 遇到下一个二级标题则退出跳过状态
        ...
        if cleaned != content.strip():
            logger.info(
                f"预处理已应用: {source}, 原长度={len(content)}, 新长度={len(cleaned)}"
            )
        return cleaned

    def _dedupe_documents(self, documents: list) -> list:
        """对分片进行去重（按内容哈希），避免重复模板导致召回噪声。"""
        seen: set[str] = set()
        uniq = []
        for doc in documents:
            ...
            seen.add(key)
            uniq.append(doc)
        if len(uniq) != len(documents):
            logger.info(f"分片去重: {len(documents)} -> {len(uniq)}")
        return uniq
    ...
    def index_single_file(self, file_path: str):
        ...
        try:
            ...
            # 1.1 入库前预处理（清理模拟/演示段落与模板模块）
            content = self._preprocess_content(content, source=str(path))
